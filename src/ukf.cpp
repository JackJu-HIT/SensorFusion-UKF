...
void UKF::ProcessMeasurement(MeasurementPackage meas_package) {
	...
	//set state dimension
	 ...
	 //set augmented dimension
	  ...
	  P_aug.fill(0.0);
	  P_aug.topLeftCorner(5,5) = P_;//左上方填充P_
	  P_aug(5,5) = std_a_*std_a_;
	  P_aug(6,6) = std_yawdd_*std_yawdd_;

	  //create square root matrix 开根号
	  MatrixXd L = P_aug.llt().matrixL();

	  //create augmented sigma points产生随机点
	  Xsig_aug.col(0)  = x_aug;
	  for (int i = 0; i< n_aug; i++)
	  {
	    Xsig_aug.col(i+1)       = x_aug + sqrt(lambda+n_aug) * L.col(i);
	    Xsig_aug.col(i+1+n_aug) = x_aug - sqrt(lambda+n_aug) * L.col(i);//L.col如何理解？
	  }
	  ...
	  //angle normalization将角度限制在-π到π之间。
...
//需要进行重新生成点。处理人：Jack Ju
//start

VectorXd x_aug_new  = VectorXd(n_x);
x_aug_new = x_;
MatrixXd P_aug_new= MatrixXd(n_x, n_x );
P_aug_new=P_;

MatrixXd Xsig_aug_new = MatrixXd(n_x, 2 * n_x + 1);
        //create square root matrix 开根号
	  MatrixXd L = P_aug_new.llt().matrixL();

	  //create augmented sigma points产生随机点
	  Xsig_aug_new.col(0)  = x_aug_new;
	  for (int i = 0; i< n_x; i++)
	  {
	    Xsig_aug_new.col(i+1)       = x_aug_new + sqrt(lambda+n_x) * L.col(i);
	    Xsig_aug_new.col(i+1+n_x) = x_aug_new - sqrt(lambda+n_x) * L.col(i);//L.col如何理解？
	  }
...
//end
		  ...
		  VectorXd weights = VectorXd(2*n_x+1);
		   double weight_0 = lambda/(lambda+n_x);
		  weights(0) = weight_0;
		  ...
		  MatrixXd Zsig = MatrixXd(n_z, 2 * n_x + 1);

		  //transform sigma points into measurement space
			for (int i = 0; i < 2 * n_x + 1; i++) {  //2n+1 simga points

			  // extract values for better readibility
			  double p_x =Xsig_aug_new(0,i);    //Xsig_pred(0,i);//被Jack Ju动了
			  double p_y =Xsig_aug_new(1,i);    //Xsig_pred(1,i);
			  double v  =  Xsig_aug_new(2,i);   //Xsig_pred(2,i);
			  double yaw =  Xsig_aug_new(3,i);  //Xsig_pred(3,i);
			...
			float rhodot = meas_package.raw_measurements_(2);
			 ...
			 Tc = Tc + weights(i) * x_diff * z_diff.transpose();//协方差
			 }
